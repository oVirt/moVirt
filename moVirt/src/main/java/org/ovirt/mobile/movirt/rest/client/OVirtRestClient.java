...
public interface OVirtRestClient extends RestClientRootUrl, RestClientHeaders, RestClientSupport {
    ...
    @Get("/events;max=" + Constants.MAX_EVENTS_PER_ENTITY + "?search=event_host={hostName}")
    Events getHostEvents(@Path String hostName);

    @Get("/events;max=" + Constants.MAX_EVENTS_PER_ENTITY + "?search=event_storage={storageDomainName}")
